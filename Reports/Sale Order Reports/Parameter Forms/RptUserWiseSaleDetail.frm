...
Begin VB.Form RptUserWiseSaleDetail 
   AutoRedraw      =   -1  'True
   ...
   ClientWidth     =   12000
   ControlBox      =   0   'False
   Icon            =   "RptUserWiseSaleDetail.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ...
   Begin JeweledBut.JeweledButton BtnClose 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "RptUserWiseSaleDetail.frx":0ECA
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnPreview 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "RptUserWiseSaleDetail.frx":0EE6
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnPrint 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "RptUserWiseSaleDetail.frx":0F02
      BC              =   14737632
      FC              =   0
   End
   ...
   Begin JeweledBut.JeweledButton BtnStore 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "RptUserWiseSaleDetail.frx":0F1E
      BC              =   12632256
      FC              =   0
   End
   ...
   End
End
Attribute VB_Name = "RptUserWiseSaleDetail"
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
   ...
   ElseIf KeyCode = vbKeyF1 Then
      Select Case ActiveControl.Name
         Case TxtStoreID.Name: If FunSelectStore(ssFunctionKey, True) = True Then CmbUsers.SetFocus
      End Select
   End If
   Exit Sub
ErrorHandler:
...
Private Sub Form_Load()
   ...
   SetWindowText Me.hWnd, "Sale Detail (User Wise)"
   DtpTo.DateValue = Date
   DtpFrom.DateValue = Date - 30
   If ObjUserSecurity.IsAdministrator = False Then
      With CN.Execute("select userno, username from users where userno = " & ObjUserSecurity.UserNo)
         If .RecordCount > 0 Then
      ...
      End With
   Else
      With CN.Execute("select userno, username from users")
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
   ...
   For Each frmObj In Forms
       Set frmObj = Nothing
   Next
   Set RptUserWiseSaleDetail = Nothing
Exit Sub
ErrorHandler:
   Call ShowErrorMessage
End Sub
...
Private Function SetReport() As Boolean
   ...
   Set RsReport = CN.Execute("EXEC ProdRptUserWiseSaleDetail '" & DtpFrom.DateValue & "','" & DtpTo.DateValue & "','" & IIf(ObjUserSecurity.IsAdministrator = False, IIf(CmbUsers.ListCount > 0, CmbUsers.ItemData(CmbUsers.ListIndex), ""), IIf(CmbUsers.ListIndex > 0, CmbUsers.ItemData(CmbUsers.ListIndex), "")) & "','" & IIf(Trim(TxtStoreID.Text) = "", "", TxtStoreID.Text) & "'")
   Set RptReportViewer.Report = New CrptUserWiseSaleDetail
   If RsReport.BOF Then
       MsgBox "No record exists.", vbInformation, Me.Caption
       Me.MousePointer = vbDefault
       Exit Function
   End If
   RptReportViewer.Report.ReportTitle = "Sale Detail (User Wise)"
