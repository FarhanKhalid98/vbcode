...
'        CN.Execute "Exec ProdActivityLog 'Login'," & Me.ParaOutUserNo & ",1," & Me.ParaOutUserNo
        If ObjRegistry.UseBin Then
         sSql = "Select Top 1 * from " & vBinDataBase & ".dbo.ActivityLogBin Where UserNo = " & !UserNo & " order by ActivityID Desc"
         With CN.Execute(sSql)
            If .RecordCount <> 0 Then
               If (!ActionNo <> eLogOut) Then
                  sSql = "insert into " & vBinDataBase & ".dbo.ActivityLogBin(ActivityDate,ActionNo,userno,FormNo, TempID, TransactionID,TransactionDate,TransactionInfo) values(getdate()," & eReLogin & "," & !UserNo & ",'" & eFrmLogin & "'," & "Null" & "," & "Null" & "," & "Null" & ",'Multi LogIn Or ReLogIn (" & LocalComputerName & ") After (Power OFF/End Task) at " & Now & "')"
                  CN.Execute sSql
                  vIsMultiLogIn = True
               End If
            End If
         End With
         If vIsMultiLogIn = False Then
            sSql = "insert into " & vBinDataBase & ".dbo.ActivityLogBin(ActivityDate,ActionNo,userno,FormNo, TempID, TransactionID,TransactionDate,TransactionInfo) values(getdate()," & eLogIn & "," & !UserNo & ",'" & eFrmLogin & "'," & "Null" & "," & "Null" & "," & "Null" & ",'Log In (" & LocalComputerName & ") Sucessfully at " & Now & "')"
            CN.Execute sSql
         End If
        End If
