...
Begin VB.Form DefPackageDeal 
   ...
   Begin JeweledBut.JeweledButton BtnPackageDeal 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "DefPackageDeal.frx":1052
      BC              =   12632256
      FC              =   0
   End
   Begin SITextBox.Txt TxtPackageDealName 
      Height          =   315
      Left            =   5325
      TabIndex        =   17
      Top             =   2085
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      BeginProperty stylesets(0).Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      stylesets(0).Picture=   "DefPackageDeal.frx":106E
      AllowUpdate     =   0   'False
   ...
   End
End
Attribute VB_Name = "DefPackageDeal"
...
Private Function FunSelectUnion(ByVal CallerName As SelectAccountCaller, ByVal IsOptional As Boolean) As Boolean
   On Error GoTo ErrorHandler
   Dim vStrSQL As String
   If CallerName = ssButton Or CallerName = ssFunctionKey Then
      SchProduct.Show vbModal, Me
      If SchProduct.ParaOutID = "" Then FunSelectUnion = False: Exit Function
      TxtUnionID.Text = SchProduct.ParaOutID
   End If
    '---------------------------
    If Trim(TxtUnionID.Text) = "" Then Exit Function
    If Len(TxtUnionID.Text) <= 5 Then
      TxtUnionID.Text = Right("00000" + CStr(Val(TxtUnionID.Text)), 5)
    End If
    If TxtUnionID.Text = "" Then FunSelectUnion = False: Exit Function
    vStrSQL = " SELECT Productid, ProductName" & vbCrLf _
           + " from Products " & vbCrLf _
           + " where productid = '" & TxtUnionID.Text & "'"
  
   With CN.Execute(vStrSQL)
      If .RecordCount > 0 Then
         TxtUnionName.Text = !ProductName
         If BtnSave.Enabled = False Then FormStatus = ChangeMode
         FunSelectUnion = True
         .Close
         Exit Function
      Else
         FunSelectUnion = False
         .Close
         TxtUnionID.Text = ""
         TxtUnionName.Text = ""
         If BtnSave.Enabled = False Then FormStatus = ChangeMode
...
Private Sub BtnClear_Click()
   On Error GoTo ErrorHandler
     CN.Execute ("Insert Into UserActivities values ('Union Info'" & "," & TxtID.Text & ",Null,'Cleared','" & Date & "','" & Time & "',6,'Cleared'," & vUser & ")")
         FormStatus = NewMode
   Exit Sub
ErrorHandler:
   Call ShowErrorMessage
...
Private Sub BtnClose_Click()
   '''''''''''''''''''''''''''''''''''''User Activities'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
   CN.Execute ("Insert Into UserActivities values ('Union Info'" & "," & TxtID.Text & ",Null,'Closed','" & Date & "','" & Time & "',7,'Closed'," & vUser & ")")
   ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
   Unload Me
End Sub
...
'   CN.Execute ("Insert Into Bin_ProductProcessInfoBody Select " & vMaxBinID & ",'" & Date & "', * from ProductProcessInfoBody Where ID = " & TxtID.Text)
   ...
   '''''''''''''''''''''''''''''''''''''User Activities'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
   CN.Execute ("Insert Into UserActivities values ('Union Info'" & "," & TxtID.Text & ",Null,'Removed','" & Date & "','" & Time & "',3,'Removed'," & vUser & ")")
   ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
   Grid.Redraw = False
   Grid.RemoveAll
   Call ActivityLog("Union Info", eDelete, TxtUnionID.Text)
   CN.Execute "Delete from UnionInfoBody where ID= " & (TxtID.Text)
   CN.Execute "Delete from UnionInfoHeader where ID= " & (TxtID.Text)
   Grid.Redraw = True
   CN.CommitTrans
   FormStatus = NewMode
   Exit Sub
...
Private Sub BtnUnion_Click()
    If FunSelectUnion(ssButton, True) = True Then
      TxtCode.SetFocus
   Else
      TxtUnionID.SetFocus
   End If
...
Private Sub BtnOpen_Click()
   SchUnionInfo.Show vbModal
   If SchUnionInfo.ParaOutID = "" Then Exit Sub
   TxtID.Text = SchUnionInfo.ParaOutID
   CN.Execute ("Insert Into UserActivities values ('Union Info'" & "," & TxtID.Text & ",Null,'Opened','" & Date & "','" & Time & "',4,'Opened'," & vUser & ")")
   GetUnionInfo
End Sub
...
'  End If
'  Header Validation
   If Trim(TxtUnionID.Text) = "" Then
      MsgBox "Enter Union ID.", vbExclamation, Me.Caption
      TxtUnionID.SetFocus
      Exit Sub
   End If
   If vIsNewRecord = True Then
      If CN.Execute("select * from UnionInfoHeader where UnionID=" & TxtUnionID.Text).RecordCount > 0 Then
         MsgBox "Union ID Already Exist.", vbExclamation, Me.Caption
         TxtUnionID.SetFocus
         Exit Sub
      End If
   End If
   ...
   CN.BeginTrans
   Call UserActivities
   ssql = "select * from UnionInfoHeader where ID=" & Val(TxtID.Text)
   ...
   With Rs
      ...
      If .BOF Then
         ...
         !id = Val(TxtID.Text)
      End If
      !UnionID = TxtUnionID.Text
      .Update
      .Close
   End With
   If vIsNewRecord = False Then Call ActivityLog("Union Info", eEdit, TxtUnionID.Text)
  'Body Validation
  ' validation has been performed when a row is added to the grid
   With RsBody
      ...
      .UpdateBatch
   End With
   If vIsNewRecord = True Then Call ActivityLog("Union Info", eAdd, TxtUnionID.Text)
   CN.CommitTrans
   FormStatus = NewMode
   Exit Sub
ErrorHandler:
...
Private Sub PopulateDataToGrid()
   ...
   RsBody.Open "Select * from UnionInfoBody where ID=" & Val(TxtID.Text), CN, adOpenStatic, adLockBatchOptimistic
   If RsBody.RecordCount > 0 Then
      ssql = "select p.productname, code, b.* from UnionInfoBody b join products p on p.productid = b.productid where ID=" & Val(TxtID.Text)
...
Private Property Let FormStatus(ByVal vNewValue As FormMode)
   ...
   Case Is = NewMode
      ...
      TxtUnionID.Enabled = True
      BtnUnion.Enabled = True
      If TxtUnionID.Enabled And TxtUnionID.Visible Then TxtUnionID.SetFocus
      vIsNewRecord = True
   Case Is = OpenMode
      BtnOpen.Enabled = True
...
'            KeyCode = 0
   ...
   ElseIf KeyCode = vbKeyF1 Then
      Select Case ActiveControl.Name
         Case TxtUnionID.Name: If FunSelectUnion(ssFunctionKey, False) = True Then TxtCode.SetFocus
...
Private Sub Form_Load()
   ...
   SetWindowText Me.hWnd, "Union Info"
   HelpLocation Me
   FormStatus = NewMode
...
Private Sub mniRemoveRow_Click()
   ...
   CN.Execute ("Insert Into UserActivities values ('Union Info'" & "," & TxtID.Text & ",Null,'Removed ProductID-" & Grid.Columns("Code").Text & " QtyLoose- " & Grid.Columns("Qty").Text & "','" & Date & "','" & Time & "',2,'Updated'," & vUser & ")")
...
Private Sub GetUnionInfo()
   On Error GoTo ErrorHandler
   ssql = "select UnionID, ProductName FROM UnionInfoHeader h join products p on p.productid = h.Unionid where h.ID=" & Val(TxtID.Text)
   With CN.Execute(ssql)
      If Not .BOF Then
          TxtUnionID.Text = !UnionID
          TxtUnionName.Text = !ProductName
      End If
      .Close
   End With
...
Private Sub TxtUnionID_Change()
   If TxtUnionID.Visible = False Then Exit Sub
   If ActiveControl.Name <> TxtUnionID.Name Then Exit Sub
   If TxtUnionName.Text <> "" Then
      TxtUnionName.Text = ""
   End If
End Sub

Private Sub TxtUnionID_Validate(Cancel As Boolean)
   If Me.ActiveControl.Name <> TxtUnionID.Name Then Exit Sub
   On Error GoTo ErrorHandler
   If TxtUnionName.Text <> "" Then Exit Sub
   Dim vTemp As Boolean
   vTemp = Not FunSelectUnion(ssValidate, True)
   If vTemp = True Then
      vTemp = Not FunSelectUnion(ssButton, False)
   End If
   Cancel = vTemp
   Exit Sub
ErrorHandler:
...
Private Function FunGetMaxID() As Long
   On Error GoTo ErrorHandler
   FunGetMaxID = CN.Execute("Select isnull(max(ID),0)+1 from UnionInfoHeader").Fields(0)
   Exit Function
ErrorHandler:
   Call ShowErrorMessage
End Function
...
Private Sub UserActivities()
    ...
    With CN.Execute("Select  * from   UnionInfoHeader where ID =" & TxtID.Text)
        If TxtID.Text <> !id Then
            CN.Execute ("Insert Into UserActivities values ('Union Info'" & "," & TxtID.Text & ", Null, 'Updated UnionID- " & !UnionID & "','" & Date & "','" & Time & "',2,'Updated'," & vUser & ")")
        End If
    End With
    Grid.MoveFirst
    For i = 1 To Grid.Rows - 1
        With CN.Execute("Select * from UnionInfoBody Where ID = " & TxtID.Text & " And Code = '" & Grid.Columns("Code").Text & "'")
             If .EOF = True Then
                CN.Execute ("Insert Into UserActivities values ('Union Info'" & "," & TxtID.Text & ",Null,'Inserted New ProductID- " & Grid.Columns("code").Text & " QtyLoose-" & Grid.Columns("Qty").Text & "','" & Date & "','" & Time & "',2,'Updated'," & vUser & ")")
             Else
                If Grid.Columns("Qty").Text <> !QtyLoose Then
                   CN.Execute ("Insert Into UserActivities values ('Union Info'" & "," & TxtID.Text & ",Null,'Updated ProductID- " & Grid.Columns("code").Text & " QtyLoose-" & !QtyLoose & "','" & Date & "','" & Time & "',2,'Updated'," & vUser & ")")
                End If
            End If
        End With
    Grid.MoveNext
    Next
   Else
    CN.Execute ("Insert Into UserActivities values ('Union Info'" & "," & TxtID.Text & ",Null,'Saved','" & Date & "','" & Time & "',1,'Saved'," & vUser & ")")
   End If
End Sub
