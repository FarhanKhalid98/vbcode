...
Begin VB.Form SchOpeningInvoice 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00F8E8D6&
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      EndProperty
      stylesets(0).Picture=   "SchOpeningStock.frx":0000
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
      ...
      Columns(1).Caption=   "Opening Date"
      Columns(1).Name =   "Date"
   ...
   Begin JeweledBut.JeweledButton BtnSelect 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "SchOpeningStock.frx":001C
      BC              =   12632256
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnClose 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "SchOpeningStock.frx":0038
      BC              =   12632256
      FC              =   0
   End
   ...
   End
End
Attribute VB_Name = "SchOpeningInvoice"
...
Public ParaOutOpeningID As Integer
Public ParaOutOpeningDate As String

Private Sub LoadGrid()
   On Error GoTo ErrorHandler
   Set Rs = New ADODB.Recordset
   VStrSQL = "SELECT h.OpeningID as ID, h.OpeningDate as Date " & vbCrLf _
         + " , StoreName, TotalAmount,TotalItems, UserName" & vbCrLf _
         + " FROM OpeningInvoiceHeader h INNER JOIN" & vbCrLf _
         + " (SELECT OpeningID, OpeningDate, sum((qtypack*multiplier)+ qtyloose) as TotalItems FROM OpeningStock GROUP BY OpeningID, OpeningDate) b" & vbCrLf _
         + " ON h.OpeningID = b.OpeningID and h.OpeningDate = b.OpeningDate" & vbCrLf _
         + " INNER JOIN Stores s  ON s.StoreID = h.StoreID " & vbCrLf _
         + " INNER JOIN users u ON h.userno = u.userno " & vbCrLf _
         + " WHERE 1=1 " & IIf(vSessionID = 0, "", " and SessionID = " & vSessionID) & vOrder & vDirection
...
Private Sub BtnClose_Click()
  Me.ParaOutOpeningID = 0
  Me.ParaOutOpeningDate = ""
  Unload Me
End Sub
...
Private Sub BtnSelect_Click()
  ...
  Me.ParaOutOpeningID = Rs!id
  Me.ParaOutOpeningDate = Rs!Date
  Unload Me
  Exit Sub
ErrorHandler:
  Call ShowErrorMessage
