...
Begin VB.Form FrmAdvances 
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      BeginProperty stylesets(0).Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      stylesets(0).Picture=   "FrmAdvances.frx":1081
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
...
Private Sub BtnPrint_Click()
   ...
   vStrSQL = " select h.voucherno, voucherdate, c.accountno, accountname, b.narration, amount " & vbCrLf _
      + " from AdvanceVouchers h inner join AdvanceVouchersBody b" & vbCrLf _
   ...
   RptReportViewer.Report.ReportTitle = "Advance Voucher"
...
'      Call DeleteTempActivityLogBin(vRandomID)
      ...
      For vCounter = 2 To Grid.Rows
         ...
         If Trim(Grid.Columns("ID").Text) <> "" Then
           sSql = "Select AccountNo From AdvanceVouchersbody where VoucherNo=" & Val(TxtVoucherNo.Text) & " and AccountNo = " & Val(Grid.Columns("ID").Text)
            With cn.Execute(sSql)
               If .EOF Then
                  Call ActivityLogBin("", eFrmAdvanceVoucher, eClearUnSavedRecord, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Cleared Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
                  vGridRows = vGridRows - 1
               End If
            End With
         Else
         ...
         Grid.MoveNext
      Next vCounter
      If vGridRows > 0 Then Call ActivityLogBin("", eFrmAdvanceVoucher, eClearSavedRecord, TxtVoucherNo.Text, DtpVoucherDate.DateValue, vGridRows & " Account/s Cleared")
      Grid.Redraw = True
  ''''''''''''''''''
  FormStatus = NewMode
  Exit Sub
...
Private Sub BtnDelete_Click()
  On Error GoTo ErrorHandler
   ...
   ''''''''''''' User Authentication ''''''''''''''
   vUserAction = UserAuthentication("MniAdvances", vUser, ObjUserSecurity.IsAdministrator, eUserDelete)
   If vUserAction <> "" Then
  ...
  Call BinData
   Call ActivityLogBin("", eFrmAdvanceVoucher, eDelete, TxtVoucherNo.Text, DtpVoucherDate.DateValue, Grid.Rows - 1 & " Accounts/s Deleted Amount: " & Val(TxtTotalAmount.Text))
   
'  Call ActivityLog("Advance Voucher", eDelete, TxtVoucherNo.Text)
  cn.Execute "Delete from AdvanceVouchersBody where VoucherNo = " & Val(TxtVoucherNo.Text)
  cn.Execute "Delete from AdvanceVouchers WHere voucherno = " & Val(TxtVoucherNo.Text)
  cn.CommitTrans
  FormStatus = NewMode
  Exit Sub
ErrorHandler:
...
Private Sub PopulateDataToGrid()
   ...
   RsBody.Open "Select * from AdvanceVouchersBody where voucherno = " & Val(TxtVoucherNo.Text), cn, adOpenDynamic, adLockBatchOptimistic
   If RsBody.RecordCount > 0 Then
      sSql = "Select AdvanceVouchersBody.*, accountname from AdvanceVouchersBody inner join chartofaccounts on chartofaccounts.accountno = AdvanceVouchersBody.accountno where voucherno = " & Val(TxtVoucherNo.Text)
      With cn.Execute(sSql)
         Grid.Redraw = False
         Grid.MoveFirst
         Grid.RemoveAll
...
Private Sub GetVoucher()
   On Error GoTo ErrorHandler
   sSql = "Select h.*, StoreName, OrganizationName from AdvanceVouchers h left outer join Stores s on s.storeid = h.storeid left outer join Organizations o on o.OrganizationID = h.OrganizationID where voucherno = " & Val(TxtVoucherNo.Text)
   With cn.Execute(sSql)
      If Not .BOF Then
...
Private Sub BtnOpen_Click()
   SchAdvance.Show vbModal, Me
   If SchAdvance.ParaOutVoucherNo <> Empty Then
      TxtVoucherNo.Text = SchAdvance.ParaOutVoucherNo
      GetVoucher
   End If
  Exit Sub
ErrorHandler:
...
Private Sub BtnSave_Click()
   ...
   ''''''''''''' User Authentication ''''''''''''''
   vUserAction = UserAuthentication("MniAdvances", vUser, ObjUserSecurity.IsAdministrator, IIf(vIsNewRecord = True, eUserNewRecord, eUserEdit))
   If vUserAction <> "" Then
      MsgBox vUserAction, vbCritical, "Error"
      Exit Sub
   End If
   ...
   If vIsNewRecord Then
      If cn.Execute("Select * from AdvanceVouchers where voucherno = " & Val(TxtVoucherNo.Text)).RecordCount > 0 Then
  ...
  'Saving record
   ...
   Call DeleteTempActivityLogBin(vRandomID)
   If vIsNewRecord = False Then Call ActivityLogBin("", eFrmAdvanceVoucher, eEdit, TxtVoucherNo.Text, DtpVoucherDate.DateValue, "Amount: " & Val(TxtTotalAmount.Text))
      
   sSql = "Select * From AdvanceVouchers Where VoucherNo =" & Val(TxtVoucherNo.Text)
   ...
   With Rs
      ...
      .Close
   End With
   If vIsNewRecord = False Then Call ActivityLog("Advance Voucher", eEdit, TxtVoucherNo.Text)
   With RsBody
      .Filter = 0
      .MoveFirst
      ...
      .UpdateBatch
   End With
   If vIsNewRecord = True Then Call ActivityLogBin("", eFrmAdvanceVoucher, eAdd, TxtVoucherNo.Text, DtpVoucherDate.DateValue, Grid.Rows - 1 & " New Account/s Added Amount: " & Val(TxtTotalAmount.Text))
'   If vIsNewRecord = True Then Call ActivityLog("Advance Voucher", eAdd, TxtVoucherNo.Text)
   cn.CommitTrans
   FormStatus = NewMode
   Exit Sub
ErrorHandler:
...
Private Sub Form_Load()
   On Error GoTo ErrorHandler
   SetWindowText Me.hWnd, "Advance Vouchers"
   ShowPicture Me, 2
...
Private Function FunGetMaxID() As Long
  On Error GoTo ErrorHandler
  FunGetMaxID = cn.Execute("Select isnull(max(voucherno),0) from AdvanceVouchers").Fields(0) + 1
  Exit Function
ErrorHandler:
  Call ShowErrorMessage
End Function
      ...
      If Grid.Rows > 1 And Cancel = 0 Then
         ...
         For vCounter = 2 To Grid.Rows
            ...
            If Trim(Grid.Columns("ID").Text) <> "" Then
               ...
               With cn.Execute(sSql)
                  If .EOF Then
                     Call ActivityLogBin("", eFrmAdvanceVoucher, eCloseUnSavedRecord, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Closed Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
                     vGridRows = vGridRows - 1
                  End If
                  End With
            Else
            ...
            Grid.MoveNext
            Next vCounter
         If vGridRows > 0 Then Call ActivityLogBin("", eFrmAdvanceVoucher, eCloseSavedRecord, TxtVoucherNo.Text, DtpVoucherDate.DateValue, vGridRows & " Account/s Closed")
         Grid.Redraw = True
      End If
  ''''''''''''''''''
   Exit Sub
...
Private Sub mniRemoveRow_Click()
   ...
   With cn.Execute(sSql)
      If .EOF Then
         Call ActivityLogBin("", eFrmAdvanceVoucher, eRemoveRowUnSaved, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Removed Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
      Else
         Call ActivityLogBin("", eFrmAdvanceVoucher, eRemoveRow, TxtVoucherNo.Text, DtpVoucherDate.DateValue, "Removed Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
         Call ActivityLogBin(vRandomID, eFrmAdvanceVoucher, eAddTempRecord, TxtVoucherNo.Text, DtpVoucherDate.DateValue, "Pending Remove Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
      End If
   End With
...
On Error GoTo ErrorHandler
   ...
   If TxtID.Enabled Then
      If RsBody.RecordCount = 0 Then
         ...
         If vIsNewRecord = False Then Call ActivityLogBin("", eFrmAdvanceVoucher, eAddNewRowByEdit, TxtVoucherNo.Text, DtpVoucherDate.DateValue, "Add New Code-" & TxtID.Text & " Amount-" & " " & TxtAmount.Text & " " & Trim(TxtNarration.Text))
         Call ActivityLogBin(vRandomID, eFrmAdvanceVoucher, eAddTempRecord, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Pending Add New Code-" & TxtID.Text & " Amount-" & TxtAmount.Text & " " & Trim(TxtNarration.Text))
      Else
   ...
   Else
      ...
      sSql = "Select AccountNo From AdvanceVouchersbody where VoucherNo=" & Val(TxtVoucherNo.Text) & " and AccountNo = " & Val(Grid.Columns("ID").Text)
         With cn.Execute(sSql)
            If .EOF Then
               Call ActivityLogBin("", eFrmAdvanceVoucher, eEditUnSaved, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Effected Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
               Call ActivityLogBin("", eFrmAdvanceVoucher, eEditUnSaved, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Updated Code-" & TxtID.Text & " Amount-" & TxtAmount.Text & " " & Trim(TxtNarration.Text))
            Else
               Call ActivityLogBin("", eFrmAdvanceVoucher, eEdit, TxtVoucherNo.Text, DtpVoucherDate.Date, "Effected Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
               Call ActivityLogBin("", eFrmAdvanceVoucher, eEdit, TxtVoucherNo.Text, DtpVoucherDate.Date, "Updated Code-" & TxtID.Text & " Amount-" & TxtAmount.Text & " " & Trim(TxtNarration.Text))
            End If
         End With
         Call ActivityLogBin(vRandomID, eFrmAdvanceVoucher, eAddTempRecord, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Pending Update Code-" & TxtID.Text & " Amount-" & TxtAmount.Text & " " & Trim(TxtNarration.Text))
   End If
...
Private Sub TxtNarration_GotFocus()
   ...
   If Trim(TxtNarration.Text) = "" Then TxtNarration.Text = "Advance Paid "
...
On Error GoTo ErrorHandler
   If ObjRegistry.UseBin = True Then
      vStrSQL = "Insert Into " & vBinDataBase & ".dbo.AdvanceVouchersBin (BinDate, ActionNo, FormNo, ActionUserNo, " & TableHeaderFields(eFrmAdvanceVoucher) & ")" & vbCrLf _
             & "Select '" & Now & "', " & eDelete & ", " & eFrmAdvanceVoucher & ", " & vUser & "," & TableHeaderFields(eFrmAdvanceVoucher) & " from AdvanceVouchers " & vbCrLf _
             & "Where VoucherNo = " & TxtVoucherNo.Text & " and VoucherDate = '" & DtpVoucherDate.DateValue & "' and StoreID = " & TxtStoreID.Text
      cn.Execute vStrSQL
      vStrSQL = "Insert Into " & vBinDataBase & ".dbo.AdvanceVouchersBodyBin (" & TableBodyFields(eFrmAdvanceVoucher) & ")" & vbCrLf _
             & "Select " & TableBodyFields(eFrmAdvanceVoucher) & " from AdvanceVouchersBody " & vbCrLf _
             & "Where VoucherNo = " & TxtVoucherNo.Text
      cn.Execute vStrSQL
  End If
Exit Sub
ErrorHandler:
