...
Begin VB.Form DefShifts 
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      EndProperty
      stylesets(0).Picture=   "DefShifts.frx":0000
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
...
Private Sub BtnDelete_Click()
  On Error GoTo ErrorHandler
   ...
   ''''''''''''' User Authentication ''''''''''''''
   vUserAction = UserAuthentication("MniShifts", vUser, ObjUserSecurity.IsAdministrator, eUserDelete)
   If vUserAction <> "" Then
  ...
  If MsgBox("Do you really want to remove this record?", vbYesNo + vbExclamation, "Confirmation") = vbNo Then Exit Sub
    vtbl = Common.ChildDataExists("Shifts", "ShiftID='" & Rs!ShiftID & "'", "")
    If vtbl <> "" Then
...
Private Sub BtnSave_Click()
   ...
   ''''''''''''' User Authentication ''''''''''''''
   vUserAction = UserAuthentication("MniShifts", vUser, ObjUserSecurity.IsAdministrator, IIf(vIsNewRecord = True, eUserNewRecord, eUserEdit))
   If vUserAction <> "" Then
      MsgBox vUserAction, vbCritical, "Error"
      Exit Sub
   End If
   ...
   If vIsNewRecord Then
      Rs.AddNew
      Rs!ShiftID = TxtID.Text
   End If
   Rs!ShiftName = TxtName.Text
   Rs.Update
   FormStatus = NewMode
   Exit Sub
ErrorHandler:
...
'      Exit Function
'    End If
'    If CN.Execute("Select count(*) from Shifts where ShiftID = " & TxtID.Text).Fields(0) > 0 Then
...
Private Sub Form_Load()
   ...
   SetWindowText Me.hWnd, "Shifts"
'   HelpLocation Me
   Set Rs = New ADODB.Recordset
   Rs.Open "Select * FROM Shifts", cn, adOpenStatic, adLockOptimistic
   Set Grid.DataSource = Rs
   Grid.Columns("ID").DataField = "ShiftID"
   Grid.Columns("Name").DataField = "ShiftName"
   FormStatus = NewMode
...
Private Sub TxtFilter_Change()
   ...
   Rs.Find "ShiftName like '" & TxtFilter.Text & "%'", , adSearchForward, 1
...
Private Function FunGetMaxID() As String
  On Error GoTo ErrorHandler
  FunGetMaxID = cn.Execute("Select isnull(max(ShiftID),0) + 1  from Shifts").Fields(0)
  Exit Function
ErrorHandler:
  Call ShowErrorMessage
End Function
