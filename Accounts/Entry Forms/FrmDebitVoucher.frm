...
Begin VB.Form FrmDebitVoucher 
   ...
   Begin VB.ComboBox CmbPrinters 
      Height          =   315
      ItemData        =   "FrmDebitVoucher.frx":0ECA
      Left            =   5123
      List            =   "FrmDebitVoucher.frx":0ECC
   ...
   Begin JeweledBut.JeweledButton BtnSearch 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "FrmDebitVoucher.frx":0FC1
      BC              =   12632256
      FC              =   0
   End
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      BeginProperty stylesets(0).Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      stylesets(0).Picture=   "FrmDebitVoucher.frx":0FDD
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
      ...
      Columns(5).Caption=   "Opening Credit"
   ...
   Begin JeweledBut.JeweledButton BtnOpen 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "FrmDebitVoucher.frx":0FF9
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnClear 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "FrmDebitVoucher.frx":1015
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnClose 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "FrmDebitVoucher.frx":1031
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnSave 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "FrmDebitVoucher.frx":104D
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnDelete 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "FrmDebitVoucher.frx":1069
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnPrint 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "FrmDebitVoucher.frx":1085
      BC              =   14737632
      FC              =   0
   End
   ...
   Begin JeweledBut.JeweledButton BtnStore 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "FrmDebitVoucher.frx":10A1
      BC              =   12632256
      FC              =   0
   End
   ...
   Begin JeweledBut.JeweledButton BtnOrganization 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "FrmDebitVoucher.frx":10BD
      BC              =   12632256
      FC              =   0
   End
   ...
   Begin JeweledBut.JeweledButton BtnBank 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "FrmDebitVoucher.frx":10D9
      BC              =   12632256
      FC              =   0
   End
   ...
   End
End
Attribute VB_Name = "FrmDebitVoucher"
...
Private Sub BtnPrint_Click()
   ...
   vStrSQL = " select h.voucherno, voucherdate, c.accountno, accountname + isnull(' - ' + p.city,'') as accountname, b.narration, amount, ServerEntry " & vbCrLf _
      + " from Debitvouchers h inner join Debitvouchersbody b" & vbCrLf _
...
'      Set RptReportViewer.Report = New CrpVoucherSmall
   ...
   RptReportViewer.Report.ReportTitle = "Cash Payment Voucher"
   ...
   '''''''''''''''''''''''''''''''''''''User Activities'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    CN.Execute ("Insert Into UserActivities values ('Debit Voucher'" & "," & TxtVoucherNo.Text & ",'" & DtpVoucherDate.DateValue & "','Printed','" & Date & "','" & Time & "',5,'Printed'," & vUser & ")")
   ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
'      Call DeleteTempActivityLogBin(vRandomID)
      ...
      For vCounter = 2 To Grid.Rows
         ...
         If Trim(Grid.Columns("ID").Text) <> "" Then
           sSql = "Select AccountNo From DebitVouchersbody where VoucherNo=" & Val(TxtVoucherNo.Text) & " and StoreID = " & TxtStoreID.Text & " and AccountNo = " & Val(Grid.Columns("ID").Text)
            With CN.Execute(sSql)
               If .EOF Then
                  Call ActivityLogBin("", eFrmDebitVoucher, eClearUnSavedRecord, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Cleared Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
                  vGridRows = vGridRows - 1
               End If
            End With
         Else
         ...
         Grid.MoveNext
      Next vCounter
      If vGridRows > 0 Then Call ActivityLogBin("", eFrmDebitVoucher, eClearSavedRecord, TxtVoucherNo.Text, DtpVoucherDate.DateValue, vGridRows & " Account/s Cleared")
      Grid.Redraw = True
  ''''''''''''''''''
  FormStatus = NewMode
  Exit Sub
...
Private Sub BtnDelete_Click()
  On Error GoTo ErrorHandler
   ...
   ''''''''''''' User Authentication ''''''''''''''
   vUserAction = UserAuthentication("MniCashPaymentVoucher", vUser, ObjUserSecurity.IsAdministrator, eUserDelete)
   If vUserAction <> "" Then
  ...
  '''''''''''''''''''''''Check Import / Export'''''''''''''''''''''''''''''''''
    If ObjRegistry.ShowMultiBranches = True Then
      vStrSQL = "select * from DebitVouchers where Tag is not null And SID=" & Val(TxtSID.Text) & " and Voucherdate = '" & DtpVoucherDate.DateValue & "'"
      With CN.Execute(vStrSQL)
          If Not .EOF Then
  ...
  Call BinData
  Call ActivityLogBin("", eFrmDebitVoucher, eDelete, TxtVoucherNo.Text, DtpVoucherDate.DateValue, Grid.Rows - 1 & " Accounts/s Deleted Amount: " & Val(TxtTotalAmount.Text))
   
  Call ActivityLog("Cash Payment Voucher", eDelete, TxtVoucherNo.Text)
  CN.Execute "Delete from DebitVouchersBody where SID = " & Val(TxtSID.Text)
  CN.Execute "Delete from DebitVouchers WHere SID = " & Val(TxtSID.Text)
   ...
   vMobileNo = Split(ObjRegistry.OwnerMobileNo, " ")
         For i = 0 To UBound(vMobileNo)
            ...
            If Len(vMobile) = 13 Then
               sSql = ObjUserSecurity.UserName & " " & FrmDebitVoucher.Caption & " Deleted ID:" & TxtVoucherNo.Text & vbCrLf & " Date:" & Format(DtpVoucherDate.DateValue, "dd-MMM-yyyy") & " Time: " & Time & vbCrLf & " NetAmt: " & TxtTotalAmount.Text
...
Private Sub PopulateDataToGrid()
   ...
   RsBody.Open "Select * from Debitvouchersbody where voucherno = " & Val(TxtVoucherNo.Text) & " and StoreID = " & TxtStoreID.Text, CN, adOpenDynamic, adLockBatchOptimistic
   If RsBody.RecordCount > 0 Then
      sSql = "Select Debitvouchersbody.*, accountname + isnull(' (' + p.Address + ')','') AccountName from Debitvouchersbody inner join chartofaccounts on chartofaccounts.accountno = Debitvouchersbody.accountno Left Outer join Parties p on chartofaccounts.AccountNo = p.PartyID where voucherno = " & Val(TxtVoucherNo.Text) & " and Debitvouchersbody.StoreID = " & TxtStoreID.Text & "order by SerialNo"
      With CN.Execute(sSql)
         Grid.Redraw = False
         Grid.MoveFirst
         Grid.RemoveAll
         ...
         While Not .EOF
            ...
            Grid.Columns("Debit").Value = !Amount
...
Private Sub GetVoucher()
   On Error GoTo ErrorHandler
   sSql = "Select h.*, StoreName, OrganizationName, AccountName from DebitVouchers h left outer join Stores s on s.storeid = h.storeid left outer join Organizations o on o.OrganizationID = h.OrganizationID left outer join ChartofAccounts ca on ca.AccountNo = h.BankID where h.SID = " & Val(TxtSID.Text) & IIf(vSessionID = 0, "", " and SessionID = " & vSessionID)
   With CN.Execute(sSql)
      If Not .BOF Then
...
Private Sub BtnOpen_Click()
   SchDV.Show vbModal, Me
   If SchDV.ParaOutVoucherNo <> Empty Then
      TxtVoucherNo.Text = SchDV.ParaOutVoucherNo
      TxtStoreID.Text = SchDV.ParaOutStoreID
      TxtSID.Text = SchDV.ParaOutSID
      GetVoucher
   End If
  Exit Sub
ErrorHandler:
...
Private Sub BtnSave_Click()
   ...
   ''''''''''''' User Authentication ''''''''''''''
   vUserAction = UserAuthentication("MniCashPaymentVoucher", vUser, ObjUserSecurity.IsAdministrator, IIf(vIsNewRecord = True, eUserNewRecord, eUserEdit))
   If vUserAction <> "" Then
      MsgBox vUserAction, vbCritical, "Error"
      Exit Sub
   End If
   ...
   If vIsNewRecord Then
      If CN.Execute("Select * from Debitvouchers where voucherno = " & Val(TxtVoucherNo.Text) & " and StoreID = " & TxtStoreID.Text).RecordCount > 0 Then
  ...
  ''''' Form Default Settings '''''''''''
   vPrinter = Split(CmbPrinters.Text, ",")
   sSql = "select * from FormDefaultSetting Where FormType = 'Debit Voucher' and LocalComputerName = '" & LocalComputerName & "'"
   If CN.Execute(sSql).EOF Then
      sSql = "Insert into FormDefaultSetting (LocalComputerName, FormType, Size, DeviceName, DriverName, Port, IsPreview, IsPrint ) Values ('" & LocalComputerName & "', 'Debit Voucher','" & cmbPrintType.Text & "','" & vPrinter(0) & "','" & vPrinter(1) & "','" & vPrinter(2) & "'," & ChkIsPreview.Value & "," & ChkIsPreview.Value & ")"
   Else
      sSql = "Update FormDefaultSetting set Size = '" & cmbPrintType.Text & "', DeviceName = '" & vPrinter(0) & "', DriverName = '" & vPrinter(1) & "', Port = '" & vPrinter(2) & "', IsPreview = " & ChkIsPreview.Value & ", IsPrint = " & ChkIsPrint.Value & " Where FormType = 'Debit Voucher' and LocalComputerName = '" & LocalComputerName & "'"
   End If
   CN.Execute sSql
   ''''''''''''''''''''''''''''''''''''''''''''
   CN.BeginTrans
   ...
   Call DeleteTempActivityLogBin(vRandomID)
   If vIsNewRecord = False Then Call ActivityLogBin("", eFrmDebitVoucher, eEdit, TxtVoucherNo.Text, DtpVoucherDate.DateValue, "Amount: " & Val(TxtTotalAmount.Text))
   
   sSql = "Select * From DebitVouchers Where VoucherNo =" & Val(TxtVoucherNo.Text) & " and StoreID = " & Val(TxtStoreID.Text)
...
'      !UserNo = vUser
   ...
   End With
   If vIsNewRecord = False Then Call ActivityLog("Cash Payment Voucher", eEdit, TxtVoucherNo.Text)
   With RsBody
      .Filter = 0
      .MoveFirst
   ...
   If vIsNewRecord = True Then Call ActivityLogBin("", eFrmDebitVoucher, eAdd, TxtVoucherNo.Text, DtpVoucherDate.DateValue, Grid.Rows - 1 & " New Account/s Added Amount: " & Val(TxtTotalAmount.Text))
   
'   If vIsNewRecord = True Then Call ActivityLog("Cash Payment Voucher", eAdd, TxtVoucherNo.Text)
   CN.CommitTrans
...
Private Sub Form_Load()
   On Error GoTo ErrorHandler
   SetWindowText Me.hWnd, "Cash Payment Vouchers"
   ShowPicture Me, 2
...
Private Function FunGetMaxID() As Long
   ...
   If ObjRegistry.AllowDailyBillNo = True Then
      FunGetMaxID = CN.Execute("Select isnull(max(voucherno),0) from Debitvouchers").Fields(0) + 1
   Else
      FunGetMaxID = CN.Execute("Select isnull(max(voucherno),0) from Debitvouchers where StoreID = " & TxtStoreID.Text).Fields(0) + 1
   End If
   Exit Function
ErrorHandler:
   Call ShowErrorMessage
      ...
      If Grid.Rows > 1 And Cancel = 0 Then
         ...
         For vCounter = 2 To Grid.Rows
            ...
            If Trim(Grid.Columns("ID").Text) <> "" Then
               ...
               With CN.Execute(sSql)
                  If .EOF Then
                     Call ActivityLogBin("", eFrmDebitVoucher, eCloseUnSavedRecord, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Closed Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
                     vGridRows = vGridRows - 1
                  End If
                  End With
            Else
            ...
            Grid.MoveNext
            Next vCounter
         If vGridRows > 0 Then Call ActivityLogBin("", eFrmDebitVoucher, eCloseSavedRecord, TxtVoucherNo.Text, DtpVoucherDate.DateValue, vGridRows & " Account/s Closed")
         Grid.Redraw = True
      End If
  ''''''''''''''''''
   Exit Sub
...
Private Sub Grid_BeforeDelete(Cancel As Integer, DispPromptMsg As Integer)
   On Error GoTo ErrorHandler
   DispPromptMsg = 0
   TxtTotalAmount.Text = Val(TxtTotalAmount.Text) - Grid.Columns("Debit").Value
...
Private Sub mniRemoveRow_Click()
   ...
   With CN.Execute(sSql)
      If .EOF Then
         Call ActivityLogBin("", eFrmDebitVoucher, eRemoveRowUnSaved, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Removed Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
      Else
         Call ActivityLogBin("", eFrmDebitVoucher, eRemoveRow, TxtVoucherNo.Text, DtpVoucherDate.DateValue, "Removed Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
         Call ActivityLogBin(vRandomID, eFrmDebitVoucher, eAddTempRecord, TxtVoucherNo.Text, DtpVoucherDate.DateValue, "Pending Remove Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
      End If
   End With
...
On Error GoTo ErrorHandler
   ...
   If TxtID.Enabled Then
      If RsBody.RecordCount = 0 Then
         RsBody.AddNew
         Grid.Columns("Serial").Text = Grid.Rows
         Grid.Columns("ID").Text = Trim(TxtID.Text)
         RsBody!AccountNo = Val(TxtID.Text)
         If vIsNewRecord = False Then Call ActivityLogBin("", eFrmDebitVoucher, eAddNewRowByEdit, TxtVoucherNo.Text, DtpVoucherDate.DateValue, "Add New Code-" & TxtID.Text & " Amount-" & " " & TxtAmount.Text & " " & Trim(TxtNarration.Text))
         Call ActivityLogBin(vRandomID, eFrmDebitVoucher, eAddTempRecord, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Pending Add New Code-" & TxtID.Text & " Amount-" & TxtAmount.Text & " " & Trim(TxtNarration.Text))
      Else
   ...
   Else
      ...
      sSql = "Select AccountNo From DebitVouchersbody where VoucherNo=" & Val(TxtVoucherNo.Text) & " and StoreID = " & TxtStoreID.Text & " and AccountNo = " & Val(Grid.Columns("ID").Text)
         With CN.Execute(sSql)
            If .EOF Then
               Call ActivityLogBin("", eFrmDebitVoucher, eEditUnSaved, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Effected Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
               Call ActivityLogBin("", eFrmDebitVoucher, eEditUnSaved, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Updated Code-" & TxtID.Text & " Amount-" & TxtAmount.Text & " " & Trim(TxtNarration.Text))
            Else
               Call ActivityLogBin("", eFrmDebitVoucher, eEdit, TxtVoucherNo.Text, DtpVoucherDate.Date, "Effected Code-" & Grid.Columns("ID").Text & " Amount-" & Grid.Columns("Amount").Text & " " & Trim(Grid.Columns("narration").Text))
               Call ActivityLogBin("", eFrmDebitVoucher, eEdit, TxtVoucherNo.Text, DtpVoucherDate.Date, "Updated Code-" & TxtID.Text & " Amount-" & TxtAmount.Text & " " & Trim(TxtNarration.Text))
            End If
         End With
         Call ActivityLogBin(vRandomID, eFrmDebitVoucher, eAddTempRecord, IIf(vIsNewRecord = True, "0", TxtVoucherNo.Text), IIf(vIsNewRecord = True, "01-01-1900", DtpVoucherDate.Date), "Pending Update Code-" & TxtID.Text & " Amount-" & TxtAmount.Text & " " & Trim(TxtNarration.Text))
   End If
                  ...
                  Grid.Columns("Debit").Value = Val(TxtAmount.Text)
                  RsBody!AccountNo = Grid.Columns("ID").Text
                  RsBody!Narration = Grid.Columns("narration").Text
                  RsBody!Amount = Val(Grid.Columns("Debit").Text)
                  Grid.MoveLast
                 With Grid
...
Private Sub TxtNarration_GotFocus()
   ...
   If Trim(TxtNarration.Text) = "" Then TxtNarration.Text = "Cash Paid "
...
Private Sub BtnOrganization_Click()
   If FunSelectOrganization(ssButton, False) = True Then
      If TxtBankID.Enabled Then TxtBankID.SetFocus
   Else
...
On Error GoTo ErrorHandler
   If ObjRegistry.UseBin = True Then
      vStrSQL = "Insert Into " & vBinDataBase & ".dbo.DebitVouchersBin (BinDate, ActionNo, FormNo, ActionUserNo, " & TableHeaderFields(eFrmDebitVoucher) & ")" & vbCrLf _
             & "Select '" & Now & "', " & eDelete & ", " & eFrmDebitVoucher & ", " & vUser & "," & TableHeaderFields(eFrmDebitVoucher) & " from DebitVouchers " & vbCrLf _
             & "Where VoucherNo = " & TxtVoucherNo.Text & " and VoucherDate = '" & DtpVoucherDate.DateValue & "' and StoreID = " & TxtStoreID.Text
      CN.Execute vStrSQL
      vStrSQL = "Insert Into " & vBinDataBase & ".dbo.DebitVouchersBodyBin (" & TableBodyFields(eFrmDebitVoucher) & ")" & vbCrLf _
             & "Select " & TableBodyFields(eFrmDebitVoucher) & " from DebitVouchersBody " & vbCrLf _
