...
Begin VB.Form SchClaim 
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      BeginProperty stylesets(0).Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      stylesets(0).Picture=   "SchClaim.frx":0038
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
   ...
   Begin VB.Label Label1 
      ...
      BackColor       =   &H00DEAB97&
      BackStyle       =   0  'Transparent
      Caption         =   "------- Claim From Date To Date ---------"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
...
Public ParaOutClaimID As Integer
Public ParaOutClaimDate As String

Private Sub LoadGrid()
   On Error GoTo ErrorHandler
   Set Rs = New ADODB.Recordset
   VStrSQL = "SELECT h.ClaimID, h.ClaimDate" & vbCrLf _
         + " , TotalAmount, TotalItems, UserName" & vbCrLf _
         + " FROM ExpiryClaimsHeader h INNER JOIN" & vbCrLf _
         + " (SELECT ClaimID, sum((Eqtypack*multiplier)+ Eqtyloose) + sum((Dqtypack*multiplier)+ Dqtyloose) as TotalItems FROM ExpiryClaimsBody GROUP BY ClaimID) b" & vbCrLf _
         + " ON h.ClaimID = b.ClaimID " & vbCrLf _
         + " INNER JOIN users u ON h.userno = u.userno " & vbCrLf _
         + " WHERE h.ClaimDate between '" & DtpFromDate.DateValue & "' and '" & DtpToDate.DateValue & "' " & IIf(ObjUserSecurity.IsAdministrator = False, " and h.userno=" & ObjUserSecurity.UserNo, "") & IIf(vSessionID = 0, "", " and SessionID = " & vSessionID) & vOrder & vDirection
...
Private Sub BtnSelect_Click()
  ...
  Me.ParaOutClaimID = Rs!ClaimID
  Me.ParaOutClaimDate = Rs!ClaimDate
  Unload Me
  Exit Sub
ErrorHandler:
  Call ShowErrorMessage
...
Private Sub Form_Load()
   ...
   Me.ParaOutClaimID = 0
   Me.ParaOutClaimDate = ""
   vOrder = " order by h.ClaimID"
   Grid.Columns("ID").DataField = "ClaimID"
   Grid.Columns("Date").DataField = "ClaimDate"
...
Private Sub TxtID_Change()
   ...
   Rs.Find "ClaimID = " & TxtID.Text, , adSearchForward, 1
