...
Begin VB.Form SchCustomOrder 
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      EndProperty
      stylesets(0).Picture=   "SchCustomOrder.frx":0000
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
   ...
   End
End
Attribute VB_Name = "SchCustomOrder"
...
Public ParaInShowOrder As Boolean
...
Private Sub LoadGrid()
   ...
   VStrSQL = "SELECT h.OrderID as OrderID, OrderDate," & vbCrLf _
         + " h.CustomerName, TotalAmount, Advance, TotalItems, UserName" & vbCrLf _
         ...
         + " left outer join CustomOrderDelivery d on h.orderid = d.orderid " & vbCrLf _
         + " WHERE d.OrderID is null and OrderDate between '" & DtpFromDate.DateValue & "' and '" & DtpToDate.DateValue & "'" & IIf(ObjUserSecurity.IsAdministrator = False And ObjUserSecurity.IsManager = False, " and h.userno=" & ObjUserSecurity.UserNo, "") & vPartyName & vOrder & vDirection
...
Private Sub TxtCustomerName_Change()
   On Error GoTo ErrorHandler
   vPartyName = " and h.CustomerName like '%" & TxtCustomerName.Text & "%'"
   LoadGrid
   Exit Sub
ErrorHandler:
   Call ShowErrorMessage
