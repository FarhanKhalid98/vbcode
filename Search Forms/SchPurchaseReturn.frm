...
Begin VB.Form SchPurchaseReturn 
   AutoRedraw      =   -1  'True
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      EndProperty
      stylesets(0).Picture=   "SchPurchaseReturn.frx":0000
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
   ...
   Begin JeweledBut.JeweledButton BtnSelect 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "SchPurchaseReturn.frx":001C
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnClose 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "SchPurchaseReturn.frx":0038
      BC              =   14737632
      FC              =   0
   End
   ...
   End
End
Attribute VB_Name = "SchPurchaseReturn"
...
Dim vStrSQL As String, vVendorName As String
...
Private Sub LoadGrid()
   ...
   vStrSQL = "SELECT SID, h.ReturnID as ID, h.ReturnDate as Date" & vbCrLf _
         + " , AccountName as PartyName, Amount-isnull(BillDisc,0)+isnull(OtherCharges,0) as TotalAmount,TotalItems, UserName" & vbCrLf _
         + " FROM PurchaseReturnHeader h INNER JOIN" & vbCrLf _
         + " (SELECT ReturnID, ReturnDate, sum((isnull(qtypack,0)*isnull(multiplier,0))+ qtyloose + isnull(bonus,0)) as TotalItems, Sum(Amount) Amount FROM PurchaseReturnBody GROUP BY ReturnID, ReturnDate) b" & vbCrLf _
         + " ON h.ReturnID = b.ReturnID and h.ReturnDate = b.ReturnDate" & vbCrLf _
         + " INNER JOIN ChartofAccounts ca ON h.vendorID = ca.AccountNo " & vbCrLf _
         + " INNER JOIN users u ON h.userno = u.userno " & vbCrLf _
         + " WHERE h.ReturnDate Between '" & DtpFromDate.DateValue & "' and '" & DtpToDate.DateValue & "'" & IIf(ObjUserSecurity.IsAdministrator = False And ObjUserSecurity.IsManager = False, " and h.userno=" & ObjUserSecurity.UserNo, "") & IIf(vSessionID = 0, "", " and SessionID = " & vSessionID) & vVendorName & vOrder & vDirection
...
Private Sub Form_Load()
   ...
   'DtpDate.Value = Date
   DtpToDate.DateValue = Me.ParaInReturnDate
   ...
   If vDateDiff = 0 Then DtpToDate.Visible = False
