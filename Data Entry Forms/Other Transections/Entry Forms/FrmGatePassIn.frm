...
Private Function FunSelectProduct(ByVal CallerName As SelectAccountCaller, ByVal IsOptional As Boolean) As Boolean
   ...
   End If
    ...
    vStrSQL = " SELECT p.productid, Code, ProductName, PurPrice, isnull(Cost,0) as Cost, PackingName, isnull(Multiplier,0) as Multiplier " & vbCrLf _
           ...
           + " where (p.productid = '" & TxtCode.Text & "' or code='" & TxtCode.Text & "')" & " and isLocked = 0 and isNoCostProduct = 0 and (StoreID is Null or StoreID = " & TxtFromStoreID.Text & ")"
...
Private Sub BtnDelete_Click()
   ...
   For vCounter = 1 To Grid.Rows
      If Trim(Grid.Columns("ProductID").Text) <> "" Then
         CN.Execute "Delete from GateInBody where GINID = " & Val(TxtGINID.Text) & " and ProductID ='" & Grid.Columns("Productid").Text & "' " & IIf(Val(Grid.Columns("PackingID").Text) = 0, "And PackingID is Null", "And PackingID = " & Val(Grid.Columns("PackingID").Text))
      End If
      Grid.MoveNext
   Next vCounter
   Grid.RemoveAll
   Grid.Redraw = True
   CN.Execute "Delete from GateInHeader where GINID = " & Val(TxtGINID.Text)
   CN.CommitTrans
   FormStatus = NewMode
   Exit Sub
ErrorHandler:
...
Private Sub BtnPrint_Click()
   ...
   vStrSQL = " Select H.GINID, H.GINDate, H.FromStoreID, H.ToStoreID, H.Description," & vbCrLf _
            + " B.ProductID, B.PackingID, isnull(B.QtyPack,0) as QtyPack, B.QtyLoose, UnitName," & vbCrLf _
            + " ProductName, PackingName, UserName " & vbCrLf _
            + " from GateInHeader  H" & vbCrLf _
            + " join GateInBody B on H.GINID = B.GINID" & vbCrLf _
   ...
   Set RptReportViewer.Report = New CrpGateInInvoice
...
'  Header Validation
   If vIsNewRecord = True Then
      If CN.Execute("Select * from GateInHeader where GINID = " & Val(TxtGINID.Text)).RecordCount > 0 Then
  ...
  'Saving record
   CN.BeginTrans
   sSql = "select * from GateInHeader where GINID = " & Val(TxtGINID.Text)
...
Private Sub PopulateDataToGrid()
   ...
   RsBody.Open "Select * from GateInBody where GINID=" & Val(TxtGINID.Text), CN, adOpenDynamic, adLockBatchOptimistic
   If RsBody.RecordCount > 0 Then
      sSql = "select p.productname, b.*, UnitName from GateInBody b join products p on p.productid = b.productid Left Outer Join Units u on u.UnitID = p.UnitID where GINID=" & Val(TxtGINID.Text)
...
Private Function FunGetMaxID() As Long
   On Error GoTo ErrorHandler
   FunGetMaxID = CN.Execute("Select isnull(max(GINID),0)+1 from GateInHeader").Fields(0)
   Exit Function
ErrorHandler:
   Call ShowErrorMessage
End Function
...
Private Sub GetGateIn()
   On Error GoTo ErrorHandler
   sSql = "Select h.*, OrganizationName FROM GateInHeader h left outer join Organizations o on o.OrganizationID = h.OrganizationID where h.GINID=" & Val(TxtGINID.Text)
   With CN.Execute(sSql)
      If Not .BOF Then
