...
Begin VB.Form SchManufacturedReturn 
   AutoRedraw      =   -1  'True
   BorderStyle     =   0  'None
   ClientHeight    =   9000
   ClientLeft      =   0
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      EndProperty
      stylesets(0).Picture=   "SchManufacturedReturn.frx":0000
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
   ...
   Begin JeweledBut.JeweledButton BtnSelect 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "SchManufacturedReturn.frx":001C
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnClose 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "SchManufacturedReturn.frx":0038
      BC              =   14737632
      FC              =   0
   End
   ...
   End
End
Attribute VB_Name = "SchManufacturedReturn"
...
Private Sub LoadGrid()
   ...
   vStrSQL = "SELECT h.ReturnID as ID, h.ReturnDate As Date" & vbCrLf _
         + " , StoreName, TotalAmount,TotalItems, UserName" & vbCrLf _
         + " FROM ManufacturedReturnHeader h INNER JOIN" & vbCrLf _
         + " (SELECT ReturnID, sum(QtyLoose) as TotalItems, sum(amount) as TotalAmount FROM ManufacturedReturnUsed GROUP BY ReturnID) b" & vbCrLf _
         + " ON h.ReturnID = b.ReturnID " & vbCrLf _
         + " INNER JOIN Stores s  ON s.StoreID = h.StoreID " & vbCrLf _
         + " INNER JOIN users u ON h.userno = u.userno " & vbCrLf _
         + " WHERE h.ReturnDate between '" & DtpFromDate.DateValue & "' and '" & DtpToDate.DateValue & "'" & IIf(ObjUserSecurity.IsAdministrator = False, " and h.userno=" & ObjUserSecurity.UserNo, "") & vOrder & vDirection
