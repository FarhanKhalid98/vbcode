...
'   xObj.Width = (((Len(zBarText)) * 8)) * 20  'Screen.TwipsPerPixelX
   ...
   SavePicture zObj.Image, "C:\a.bmp"
End Sub
...
'      xObj.FONTSIZE = 8
'      xObj.FontBold = False
'      xObj.FontItalic = False
'      xObj.CurrentY = -2 + BarH
'      xObj.Print Mid(xstr, 8, 6)
'
'      xObj.Font = "Arial Narrow"
'      xObj.FONTSIZE = 8
'      xObj.FontBold = False
'      xObj.FontItalic = False
'
...
Private Sub Draw_Bar(Encoding As String, Guard As Boolean)
   ...
   For ii = 1 To Len(Encoding)
      xPos = xPos + 1
      xObj.Line (xPos + 5, xtop)-(xPos + 5, xtop + BarH + IIf(Guard, 5, 0)), IIf(Mid(Encoding, ii, 1), vbBlack, vbWhite)
   Next
End Sub
