...
Public ParaInKitchenPrint As Boolean
...
Private Sub ChkKitchenPrint_Click()
   If ChkKitchenPrint.Value = 1 Then
      ParaInKitchenPrint = True
   Else
      ParaInKitchenPrint = False
   End If
End Sub

Private Sub Form_Activate()
   ChkPrint.Value = Abs(ParaInPrint)
   ChkKitchenPrint.Value = Abs(ParaInKitchenPrint)
...
Private Sub Form_Load()
   ...
   TxtCustomerID.Text = cn.Execute("Select isnull(dbo.DefaultValue('Sale Advance'),'')").Fields(0).Value
   If TxtCustomerID.Text <> "" Then Call FunSelectCustomer(ssValidate, False)
   ChkPrint.Enabled = Not ObjRegistry.HideAutoPrint
   ChkKitchenPrint.Visible = ObjRegistry.PrintKitchenInoices
End Sub
