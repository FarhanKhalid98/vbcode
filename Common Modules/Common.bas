...
Public Fragment As Integer, Chunk() As Byte, i As Long, FileName As String
...
Public vPurchaseID As String, vPurchaseDate As String, vUserAction As String
...
Public Enum EntryForm
  ...
  eFrmRecoveryCustomerWise = 8
  eFrmRecoveryInvoiceWise = 9
  eFrmSaleOrderPOS = 10
  eFrmSaleOrderDIS = 11
  eFrmPurchaseInvoice = 21
  eFrmPurchaseReturnInvoice = 22
  eFrmPaymentVender = 23
  eFrmPaymentInvoice = 24
  eFrmPurchaseOrder = 25
  eFrmCreditVoucher = 31
  eFrmDebitVoucher = 32
  eFrmJournalVoucher = 33
  eFrmAdvanceVoucher = 34
  eFrmStockWastageInvoice = 41
  eFrmStockAdjustment = 42
  eFrmProducts = 51
  eFrmChangePrice = 52
  eFrmAdminClosing = 81
  eFrmSalaries = 82
...
Public Function GetServerDate() As Date
    GetServerDate = CN.Execute("Select DATEADD(dd, 0, DATEDIFF(dd, 0, GETDATE()))").Fields(0)
End Function
...
'  If Err.Description = "Overflow" Or Err.Number = -2147217900 Then
  If Err.Description = "Overflow" Then
      Resume Next
      Exit Sub
  End If
'  MsgBox Err.Number & " " & Err.Description & " error will occur on line " & Erl, vbCritical, "Error occured"
...
Public Function FunSecurityCheck() As Boolean
   ...
   With CN.Execute("select * from Counter")
      If .RecordCount > 0 Then
         vCount = EncryptStr(.Fields(0).Value, False)
         vMin = EncryptStr(.Fields(1).Value, False)
         vCurrentDate = EncryptStr(.Fields(2).Value, False)
         vPreviousDate = EncryptStr(.Fields(3).Value, False)
         vExpiryDate = EncryptStr(.Fields(4).Value, False)
...
'      cn.Execute "Insert into Watch(ErrorFrom,Narration) values ('Security','vCurrentDate = " & vCurrentDate & ", vExpiryDate = " & vExpiryDate & "')"
      CN.Execute "update Court set Type = '" & EncryptStr("Laptop", True) & "'" ' where SID = '" & b & "'"
      FunSecurityCheck = False
      'Timer2.Enabled = False
      Exit Function
   End If
   CN.Execute "update counter set Counter = '" & EncryptStr(vCount, True) & "'"
   CN.Execute "update counter set CMin = '" & EncryptStr(vMin, True) & "'"
   CN.Execute "update Counter set CLog = '" & EncryptStr(CStr(Month(vCurrentDate) & "/" & Day(vCurrentDate) & "/" & Year(vCurrentDate)), True) & "'"
   CN.Execute "update Counter set PLog = '" & EncryptStr(CStr(Month(vPreviousDate) & "/" & Day(vPreviousDate) & "/" & Year(vPreviousDate)), True) & "'"
...
Public Function FunScan(myString As String, Falg As Boolean) As String
   Dim i As Integer, TotalASCValue As Integer, AvgTotalAscVAlue As Integer
   Dim myScan As String
   If Len(myString) Mod 2 = 0 Then myString = myString + " "
   For i = 1 To Len(myString)
      TotalASCValue = TotalASCValue + Asc(Mid(myString, i, i))
   Next
   AvgTotalAscVAlue = TotalASCValue / Len(myString)
   
   For i = 1 To Len(myString)
       myScan = Chr(AvgTotalAscVAlue + (i * i)) + Chr(Asc(Mid(myString, i, i)) + (i * i))
       FunScan = FunScan & myScan
   Next
'   For i = 1 To Len(myString)
'       If Falg = True Then
'           myScan = Chr(Asc(Mid(myString, i, i)) + Asc(i))
'       ElseIf Falg = False Then
'           myScan = Chr(Asc(Mid(myString, i, i)) - Asc(i))
'       End If
'       FunScan = FunScan & myScan
'   Next
...
Public Function UnEncryptStr(myString As String, UnEncryPt As Boolean) As String
   Dim i As Integer
   Dim myPwd As String
   For i = 1 To Len(myString)
       If UnEncryPt = True Then
           myPwd = Chr(Asc(Mid(myString, i, i)) - 70 - Asc(i))
       ElseIf UnEncryPt = False Then
           myPwd = Chr(Asc(Mid(myString, i, i)) + 70 + Asc(i))
       End If
       UnEncryptStr = UnEncryptStr & myPwd
   Next
End Function
...
Public Function UserAuthentication(vForm As String, vUserNo As Byte, vAdmin As Boolean, vAction As Byte) As String
   UserAuthentication = ""
   If vAdmin = False Then
      If vAction = eUserEdit Then
         With CN.Execute("Select * from usertasks where UserNO = " & vUserNo & " and TaskKey ='" & vForm & "' and isUserEdit = 0")
            If .EOF = False Then
               UserAuthentication = "You are not authorized to modify Record."
            End If
         End With
      ElseIf vAction = eUserDelete Then
         With CN.Execute("Select * from usertasks where UserNO = " & vUserNo & " and TaskKey ='" & vForm & "' and isUserDelete = 0")
            If .EOF = False Then
               UserAuthentication = "You are not authorized to Delete Record."
            End If
         End With
      End If
   End If
