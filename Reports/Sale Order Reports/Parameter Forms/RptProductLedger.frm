...
Private Function SetReport() As Boolean
   ...
   Set RsReport = CN.Execute("EXEC ProdRptProductLedger '" & DtpFrom.DateValue & "','" & DtpTo.DateValue & "'" & IIf(Trim(TxtProductID.Text) = "", "Null", ",'" & TxtProductID.Text) & "'")
   ...
   RptReportViewer.Report.ParameterFields(4).AddCurrentValue IIf(ObjRegistry.CompanyAddress = "", "", ObjRegistry.CompanyAddress)
   RptReportViewer.Report.ParameterFields(5).AddCurrentValue IIf(ObjRegistry.CompanyCity = "", "", ", " & ObjRegistry.CompanyCity)
