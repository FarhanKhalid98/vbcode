...
Begin VB.Form FrmDeleteSelectedSales 
   AutoRedraw      =   -1  'True
   ...
   ClientTop       =   375
   ClientWidth     =   15360
   Icon            =   "FrmDeleteSelectedSales.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      EndProperty
      stylesets(0).Picture=   "FrmDeleteSelectedSales.frx":0ECA
      ...
      BeginProperty stylesets(1).Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      stylesets(1).Picture=   "FrmDeleteSelectedSales.frx":0EE6
      MultiLine       =   0   'False
   ...
   Begin JeweledBut.JeweledButton BtnDelete 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "FrmDeleteSelectedSales.frx":0F02
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnClose 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "FrmDeleteSelectedSales.frx":0F1E
      BC              =   14737632
      FC              =   0
   End
   ...
   Begin VB.Label LblCaption 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "Delete Selected Sale Invoice"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   13.5
         Charset         =   178
   ...
   End
End
Attribute VB_Name = "FrmDeleteSelectedSales"
...
Private Sub BtnDelete_Click()
   ...
   If Grid.Columns("Lock").Value = -1 Then
      ''''''''''''' Delete Sale Body''''''''''''''''''''''''
      vStrSQL = "Delete SaleBody Where SID = " & Grid.Columns("SID").Value & " and billID = " & Grid.Columns("ID").Value & " and BillDate ='" & Grid.Columns("Date").Text & "'"
      CN.Execute (vStrSQL)
         
      ''''''''''''' Delete Sale Header ''''''''''''''''''''''''
      vStrSQL = "Delete SaleHeader Where SID = " & Grid.Columns("SID").Value & " and billID = " & Grid.Columns("ID").Value & " and BillDate ='" & Grid.Columns("Date").Text & "'"
      CN.Execute (vStrSQL)
   End If
      Grid.MoveNext
   Next vCounter
...
Private Sub LoadGrid()
   ...
   'CmbClass.ListIndex = 0
   Me.MousePointer = vbHourglass
   vStrSQL = "SELECT h.Sid, h.BillID as SaleID, h.StoreID, c.AccountNo, OrderID, h.BillDate as SaleDate, TableName, Substring(CONVERT(varchar(20),isnull(BillTime,0)),13,7) as BillTime, case when credit = 1 then 'Credit' when cash = 1 then 'Cash' when BankCard = 1 then 'Bank Card' end as BillType, InvType" & vbCrLf _
