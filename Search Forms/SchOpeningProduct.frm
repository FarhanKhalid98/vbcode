...
Begin VB.Form SchOpeningProduct 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00F8E8D6&
   ...
   ClientTop       =   375
   ClientWidth     =   15420
   Icon            =   "SchOpeningProduct.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.ComboBox CmbUsers 
      Height          =   315
      ItemData        =   "SchOpeningProduct.frx":0ECA
      Left            =   3000
      List            =   "SchOpeningProduct.frx":0ED4
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      EndProperty
      stylesets(0).Picture=   "SchOpeningProduct.frx":0EEE
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
   ...
   Begin JeweledBut.JeweledButton BtnSelect 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "SchOpeningProduct.frx":0F0A
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnClose 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "SchOpeningProduct.frx":0F26
      BC              =   14737632
      FC              =   0
   End
   ...
   End
End
Attribute VB_Name = "SchOpeningProduct"
...
Private Sub LoadGrid()
   ...
   Dim vSQL As String
   vSQL = " SELECT h.ID as ID, EntryDate, isnull(StoreName,'') StoreName, Tag, UserName, Total, case when isverify = 0 then 'Pending' Else 'Verified' end as Status" & vbCrLf _
         + " FROM OpeningProductHeader h INNER JOIN (select ID, sum(Opening)Total from OpeningProductBody group by ID)b on h.ID = b.ID " & vbCrLf _
         + " INNER JOIN users u on h.ToUserNo = u.UserNo " & vbCrLf _
