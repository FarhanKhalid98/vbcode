...
Begin VB.Form SchGIN 
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      BeginProperty stylesets(0).Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      stylesets(0).Picture=   "SchGIN.frx":0F1E
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid GridDetail 
      ...
      BeginProperty stylesets(0).Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      stylesets(0).Picture=   "SchGIN.frx":0F3A
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
      AllowRowSizing  =   0   'False
      AllowGroupSizing=   0   'False
...
Public ParaOutGINID As Long
Dim vOrder As String, vDirection As String, vCol As Byte, vSQL As String

Private Sub LoadData()
   On Error GoTo ErrorHandler
   Set Rs = New ADODB.Recordset
   vSQL = " Select h.GINID ID, GINDate as Date, isnull(Description,'') as Description, UserName" & vbCrLf _
      ...
      If ChkDate.Value = 1 Then vSQL = vSQL + " And GINDate between '" & DtpFrom.DateValue & "' AND '" & DtpTo.DateValue & "'"
...
Private Sub LoadDetail()
   On Error GoTo ErrorHandler
   vSQL = " Select ProductName, cast(QtyLoose as varchar(10)) + ' ' + isnull(Unitname,'') as Total from GatePassInHeader h" & vbCrLf _
      + " inner join GatePassInBody b on h.GINID = b.GINID" & vbCrLf _
      + " inner join Products p on p.ProductID = b.ProductID" & vbCrLf _
      + " Left Outer Join Units U on U.UnitID = p.UnitID Where 1=1 " & vbCrLf _
      + IIf(Grid.Columns("ID").Text = "", "", " And h.GINID = " & Grid.Columns("ID").Text)
