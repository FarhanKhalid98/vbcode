...
Private Sub LoadData()
  On Error GoTo ErrorHandler
    ...
    vSQL = " Select h.VochID ID, h.BankName, h.VochDate as Date, Sum(isnull((b.DepositAmount),0)) as Amount" & vbCrLf _
      + " from BankcashDepositHeader h inner join BankcashDepositbody b on h.VochID = b.VochID " & vbCrLf _
      + " Where VochDate between '" & DtpFrom.DateValue & "' AND '" & DtpTo.DateValue & "'" & vbCrLf _
      + IIf(Trim(TxtBankName.Text) = "", "", " and H.BankName like '%" & TxtBankName.Text & "%'") & vbCrLf _
      + IIf(Trim(TxtSlipNo.Text) = "", "", " and B.SlipNo like '%" & TxtSlipNo.Text & "%'") & vbCrLf _
      + " Group by h.VochID, h.BankName, Vochdate " & vOrder & vDirection
...
Private Sub LoadDetail()
  On Error GoTo ErrorHandler
    ...
    vSQL = " Select B.SlipNo, B.SlipDate, B.DepositAmount from BankcashDepositBody B" & vbCrLf _
      + " inner join BankcashDepositBody H on h.vochID = b.vochID " & vbCrLf _
