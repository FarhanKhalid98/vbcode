CREATE TRIGGER [tu_SaleReturnbody] ON [dbo].[SaleReturnBody] 
FOR UPDATE
AS
BEGIN
...
/**************************************************************************/
Select @IProductId=ProductId, @IStoreID=StoreID, @IQty=Qty , @ICost=Price-isnull(DiscPC,0), @NewTTLCost = Round(@IQty*@ICost, 6)
From INSERTED i inner join salereturnheader h  on i.returnid = h.returnid and i.returndate = h.returndate
/***************************************************/
Select @DProductId=ProductId, @DStoreId=StoreId, @DQty=Qty, @DCost=Price-isnull(DiscPC,0), @DelTTLCost = @DQty*@DCost
From DELETED d inner join salereturnheader h  on d.returnid = h.returnid and d.returndate = h.returndate
/***************************************************/
