...
Begin VB.Form SchGRN 
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      BeginProperty stylesets(0).Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      stylesets(0).Picture=   "SchGRN.frx":0038
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
      ...
      Columns(2).Caption=   "VendorID"
      Columns(2).Name =   "VendorID"
      Columns(2).DataField=   "Column 5"
      Columns(2).DataType=   8
      Columns(2).FieldLen=   256
      Columns(3).Width=   4260
      Columns(3).Caption=   "Vendor Name"
      Columns(3).Name =   "VendorName"
      Columns(3).CaptionAlignment=   2
      Columns(3).DataField=   "Column 6"
      Columns(3).DataType=   8
      Columns(3).FieldLen=   256
      Columns(4).Width=   2514
      Columns(4).Caption=   "GRN Amount"
      Columns(4).Name =   "GRNAmount"
      Columns(4).DataField=   "Column 5"
      Columns(4).DataType=   8
      Columns(4).FieldLen=   256
      Columns(5).Width=   5583
      Columns(5).Caption=   "Remarks"
      Columns(5).Name =   "Remarks"
...
Public ParaOutGRNID As String

Private Sub LoadGrid()
   On Error GoTo ErrorHandler
   Set Rs = New ADODB.Recordset
   vStrSQL = "SELECT H.GRNID as ID, H.GRNDate as Date,  H.GRNAmount, H.Remarks, H.VendorID, partyname, UserName " & vbCrLf _
         + " FROM GRN H  " & vbCrLf _
         + " INNER JOIN Parties p ON h.VendorID = P.PartyID " & vbCrLf _
         + " INNER JOIN users u ON h.userno = u.userno " & vbCrLf _
         + " WHERE 1=1 and GRNDate Between '" & DtpFrom.DateValue & "' and '" & DtpTo.DateValue & "'" & IIf(ObjUserSecurity.IsAdministrator = False And ObjUserSecurity.IsManager = False, " and h.userno=" & ObjUserSecurity.UserNo, "") & vOrder & vDirection
   Rs.Open vStrSQL, cn, adOpenStatic, adLockReadOnly
   Set Grid.DataSource = Rs
   Grid.Columns("ID").DataField = "ID"
   Grid.Columns("Date").DataField = "Date"
   Grid.Columns("VendorID").DataField = "VendorID"
   Grid.Columns("Vendorname").DataField = "partyname"
   Grid.Columns("GRNAmount").DataField = "GRNAmount"
