...
Begin VB.Form FrmGatePassOut 
   BorderStyle     =   1  'Fixed Single
   ClientHeight    =   11130
   ClientLeft      =   45
   ClientTop       =   375
   ClientWidth     =   15360
   Icon            =   "FrmGatePassOut.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   742
   ...
   Begin JeweledBut.JeweledButton BtnPrint 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "FrmGatePassOut.frx":0F56
      BC              =   14737632
      FC              =   0
   End
   Begin SITextBox.Txt TxtGOUTID 
      Height          =   315
      Left            =   2907
      TabIndex        =   0
      Top             =   2063
   ...
   Begin SITextBox.Txt TxtDescription 
      ...
      EndProperty
   End
   Begin SSCalendarWidgets_A.SSDateCombo DtpGOUTDate 
      Height          =   315
      Left            =   3957
      TabIndex        =   1
      Top             =   2063
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      BeginProperty stylesets(0).Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      stylesets(0).Picture=   "FrmGatePassOut.frx":0F8E
      AllowUpdate     =   0   'False
   ...
   End
End
Attribute VB_Name = "FrmGatePassOut"
...
Private Sub BtnDelete_Click()
   ...
   ''''''''''''' User Authentication ''''''''''''''
   vUserAction = UserAuthentication("MniGatePassOut", vUser, ObjUserSecurity.IsAdministrator, eUserDelete)
   If vUserAction <> "" Then
   ...
   For vCounter = 1 To Grid.Rows
      If Trim(Grid.Columns("ProductID").Text) <> "" Then
         cn.Execute "Delete from GatePassOutBody where GOUTID = " & Val(TxtGOUTID.Text) & " and ProductID ='" & Grid.Columns("Productid").Text & "' " & IIf(Val(Grid.Columns("PackingID").Text) = 0, "And PackingID is Null", "And PackingID = " & Val(Grid.Columns("PackingID").Text))
      End If
      Grid.MoveNext
   Next vCounter
   Grid.RemoveAll
   Grid.Redraw = True
   cn.Execute "Delete from GatePassOutHeader where GOUTID = " & Val(TxtGOUTID.Text)
   cn.CommitTrans
   FormStatus = NewMode
   Exit Sub
ErrorHandler:
...
Private Sub BtnOpen_Click()
   SchGOUT.Show vbModal
   If SchGOUT.ParaOutGOUTID <> 0 Then
      TxtGOUTID.Text = SchGOUT.ParaOutGOUTID
      GetGateIn
   End If
End Sub
...
Private Sub BtnPrint_Click()
   ...
   vStrSQL = "  Select H.GOUTID, H.GOUTDate, H.Description," & vbCrLf _
            ...
            + " join GatePassOutBody B on H.GOUTID = B.GOUTID" & vbCrLf _
            ...
            + " Left Outer Join Units ut on ut.UnitId = p.unitid where h.GOUTID = " & Val(TxtGOUTID.Text)

      
   If RsReport.State = adStateOpen Then RsReport.Close
   RsReport.Open vStrSQL, cn, adOpenDynamic, adLockReadOnly
  
   Set RptReportViewer.Report = New CrpGatePassOutInvoice
...
Private Sub BtnSave_Click()
  On Error GoTo ErrorHandler
   ...
   ''''''''''''' User Authentication ''''''''''''''
   vUserAction = UserAuthentication("MniGatePassOut", vUser, ObjUserSecurity.IsAdministrator, IIf(vIsNewRecord = True, eUserNewRecord, eUserEdit))
   If vUserAction <> "" Then
      MsgBox vUserAction, vbCritical, "Error"
      Exit Sub
   End If
...
'  Header Validation
   If vIsNewRecord = True Then
      If cn.Execute("Select * from GatePassOutHeader where GOUTID = " & Val(TxtGOUTID.Text)).RecordCount > 0 Then
         'MsgBox "This Bill ID already exists. A new Bill ID. has been generated. Please try again", vbCritical, "Alert"
         TxtGOUTID.Text = FunGetMaxID
         'Exit Sub
      End If
   End If
  RsBody.Filter = 0
  ...
  'Saving record
   cn.BeginTrans
   ssql = "select * from GatePassOutHeader where GOUTID = " & Val(TxtGOUTID.Text)
   Dim Rs As New ADODB.Recordset
   With Rs
      .Open ssql, cn, adOpenDynamic, adLockOptimistic
      If .BOF Then
         .AddNew
         !GOUTID = Val(TxtGOUTID.Text)
      End If
      !GOUTDate = DtpGOUTDate.DateValue
...
Private Sub PopulateDataToGrid()
   ...
   RsBody.Open "Select * from GatePassOutBody where GOUTID=" & Val(TxtGOUTID.Text), cn, adOpenDynamic, adLockBatchOptimistic
   If RsBody.RecordCount > 0 Then
      ssql = "select p.productname, b.*, UnitName from GatePassOutBody b join products p on p.productid = b.productid Left Outer Join Units u on u.UnitID = p.UnitID where GOUTID=" & Val(TxtGOUTID.Text)
...
Private Property Let FormStatus(ByVal vNewValue As FormMode)
   ...
   Case Is = NewMode
      ...
      TxtGOUTID.Text = FunGetMaxID()
      If DtpGOUTDate.Enabled And DtpGOUTDate.Visible Then DtpGOUTDate.SetFocus
      vIsNewRecord = True
   Case Is = OpenMode
      BtnOpen.Enabled = True
...
Private Function FunGetMaxID() As Long
   On Error GoTo ErrorHandler
   FunGetMaxID = cn.Execute("Select isnull(max(GOUTID),0)+1 from GatePassOutHeader").Fields(0)
   Exit Function
ErrorHandler:
   Call ShowErrorMessage
End Function
...
Private Sub GetGateIn()
   On Error GoTo ErrorHandler
   ssql = "Select h.*, OrganizationName FROM GatePassOutHeader h left outer join Organizations o on o.OrganizationID = h.OrganizationID where h.GOUTID=" & Val(TxtGOUTID.Text)
   With cn.Execute(ssql)
      If Not .BOF Then
          DtpGOUTDate.Date = !GOUTDate
