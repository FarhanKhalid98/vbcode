...
Begin VB.Form DefAreas 
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      EndProperty
      stylesets(0).Picture=   "DefArea.frx":0ECA
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
...
Private Sub BtnDelete_Click()
  ...
  If MsgBox("Do you really want to remove this record?", vbYesNo + vbExclamation, "Confirmation") = vbNo Then Exit Sub
    vtbl = Common.ChildDataExists("Towns", "TownId='" & Rs!TownId & "'", "")
    If vtbl <> "" Then
...
Private Function FunValidation() As Boolean
  ...
  If Trim(TxtName.Text) = "" Then
    MsgBox "Please specify a Town name", vbExclamation, "Alert"
...
Private Sub Form_Load()
   On Error GoTo ErrorHandler
   SetWindowText Me.hWnd, "Towns"
   ShowPicture Me, 2
   AddLabelEffect Me, 2, vbWhite, vbBlack, lblEffectBorder
   Set Rs = New ADODB.Recordset
   Rs.Open "Select * FROM Towns", CN, adOpenDynamic, adLockOptimistic
...
Private Sub TxtFilter_Change()
   ...
   Rs.Find "TownName like '" & TxtFilter.Text & "%'", , adSearchForward, 1
...
Private Function FunGetMaxID() As Integer
  On Error GoTo ErrorHandler
  FunGetMaxID = CN.Execute("Select (isnull(max(TownId),0) + 1) from Towns").Fields(0)
  Exit Function
ErrorHandler:
  Call ShowErrorMessage
End Function
