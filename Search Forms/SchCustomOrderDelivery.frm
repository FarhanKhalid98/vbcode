...
Begin VB.Form SchCustomOrderDelivery 
   AutoRedraw      =   -1  'True
   ...
   Begin VB.TextBox TxtCustomerName 
      ...
      Width           =   2595
   End
   Begin VB.TextBox TxtDeliveryID 
      Appearance      =   0  'Flat
      BackColor       =   &H00FFFFFF&
      Height          =   330
      Left            =   1778
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      EndProperty
      stylesets(0).Picture=   "SchCustomOrderDelivery.frx":0000
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
      ...
      Columns(1).Caption=   "Delivery Date"
      Columns(1).Name =   "Date"
   ...
   Begin JeweledBut.JeweledButton BtnSelect 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "SchCustomOrderDelivery.frx":001C
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnClose 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "SchCustomOrderDelivery.frx":0038
      BC              =   14737632
      FC              =   0
   End
   ...
   End
End
Attribute VB_Name = "SchCustomOrderDelivery"
...
Public ParaOutDeliveryID As Long

Private Sub LoadGrid()
   On Error GoTo ErrorHandler
   If DtpFromDate.IsDateValid = False Then Exit Sub
   Set Rs = New ADODB.Recordset
   VStrSQL = " SELECT DeliveryID, DeliveryDate, case when d.credit = 1 then 'Credit' when d.cash = 1 then 'Cash' when d.BankCard = 1 then 'Bank Card' end as PaymentType" & vbCrLf _
         + " , d.CustomerName, TotalAmount, Payment, TotalItems, UserName" & vbCrLf _
         + " FROM CustomOrderDelivery D " & vbCrLf _
         ...
         + " WHERE DeliveryDate between '" & DtpFromDate.DateValue & "' and '" & DtpToDate.DateValue & "'" & IIf(ObjUserSecurity.IsAdministrator = False And ObjUserSecurity.IsManager = False, " and h.userno=" & ObjUserSecurity.UserNo, "") & vPartyName & vOrder & vDirection
   
   Rs.Open VStrSQL, CN, adOpenStatic, adLockReadOnly
   Set Grid.DataSource = Rs
   Grid.Columns("ID").DataField = "DeliveryID"
   Grid.Columns("Date").DataField = "DeliveryDate"
...
Private Sub BtnSelect_Click()
   ...
   Me.ParaOutDeliveryID = Rs!DeliveryID
   Unload Me
   Exit Sub
ErrorHandler:
   Call ShowErrorMessage
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
   ...
   If KeyCode = vbKeyReturn Then
      ...
      Case Grid.Name, TxtDeliveryID.Name, DtpFromDate.Name, DtpToDate.Name
...
Private Sub Grid_KeyPress(KeyAscii As Integer)
   ...
   Case vbKey0 To vbKey9
      TxtDeliveryID.Text = Chr(KeyAscii): TxtDeliveryID.SelStart = Len(TxtDeliveryID.Text): TxtDeliveryID.SetFocus
   End Select
End Sub
...
Private Sub TxtDeliveryID_Change()
   On Error GoTo ErrorHandler
   If Trim(TxtDeliveryID.Text) = "" Then Grid.MoveFirst: Exit Sub
   Rs.Find "DeliveryID = " & TxtDeliveryID.Text, , adSearchForward, 1
