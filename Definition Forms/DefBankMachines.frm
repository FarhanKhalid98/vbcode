...
Begin VB.Form DefBankMachines 
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      BeginProperty stylesets(0).Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      stylesets(0).Picture=   "DefBankMachines.frx":0F55
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
   ...
   Begin JeweledBut.JeweledButton BtnNew 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "DefBankMachines.frx":0F71
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnOpen 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "DefBankMachines.frx":0F8D
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnDelete 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "DefBankMachines.frx":0FA9
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnSave 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "DefBankMachines.frx":0FC5
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnClear 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "DefBankMachines.frx":0FE1
      BC              =   14737632
      FC              =   0
   End
   Begin JeweledBut.JeweledButton BtnClose 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "DefBankMachines.frx":0FFD
      BC              =   14737632
      FC              =   0
   End
   ...
   Begin JeweledBut.JeweledButton BtnAccount 
      ...
      FOCUSR          =   -1  'True
      MPTR            =   0
      MICON           =   "DefBankMachines.frx":1019
      BC              =   14737632
      FC              =   0
   End
   ...
   End
End
Attribute VB_Name = "DefBankMachines"
...
Private Sub BtnClear_Click()
    '''''''''''''''''''''''''''''''''''''User Activities'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    cn.Execute ("Insert Into UserActivities values ('BankMachines'" & "," & TxtID.Text & ",Null,'Cleared','" & Date & "','" & Time & "',6,'Cleared'," & vUser & ")")
    ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
Private Sub BtnClose_Click()
    '''''''''''''''''''''''''''''''''''''User Activities'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    cn.Execute ("Insert Into UserActivities values ('BankMachines'" & "," & TxtID.Text & ",Null,'Closed','" & Date & "','" & Time & "',7,'Closed'," & vUser & ")")
    ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    Unload Me
End Sub
...
Private Sub BtnDelete_Click()
  On Error GoTo ErrorHandler
   ...
   ''''''''''''' User Authentication ''''''''''''''
   vUserAction = UserAuthentication("MniBankMachines", vUser, ObjUserSecurity.IsAdministrator, eUserDelete)
   If vUserAction <> "" Then
  ...
  If Rs.RecordCount > 0 Then
    ...
    vtbl = Common.ChildDataExists("BankMachines", "BankMachineID='" & Rs!BankMachineID & "'", "")
    If vtbl <> "" Then
      MsgBox "The record cannot be deleted because it exists in table : " & vtbl, vbCritical, "Error"
      Exit Sub
    End If
    Call ActivityLog("Bank Machine", eDelete, TxtID.Text)
    '''''''''''''''''''''''''''''''''''''User Activities'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    cn.Execute ("Insert Into UserActivities values ('BankMachines'" & "," & TxtID.Text & ",Null,'Removed','" & Date & "','" & Time & "',3,'Removed'," & vUser & ")")
    ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    Rs.Delete
...
Private Sub BtnSave_Click()
   ...
   ''''''''''''' User Authentication ''''''''''''''
   vUserAction = UserAuthentication("MniBankMachines", vUser, ObjUserSecurity.IsAdministrator, IIf(vIsNewRecord = True, eUserNewRecord, eUserEdit))
   If vUserAction <> "" Then
      MsgBox vUserAction, vbCritical, "Error"
      Exit Sub
   End If
   ...
   If vIsNewRecord = False Then Call ActivityLog("Bank Machine", eEdit, TxtID.Text)
   
   Call UserActivities
   
   Rs.Filter = "BankMachineID = " & TxtID.Text
   If vIsNewRecord Then
      Rs.AddNew
      Rs!BankMachineID = TxtID.Text
   End If
   Rs!BankMachineName = TxtName.Text
   Rs!AccountNo = TxtAccountNo.Text
   Rs!Commision = Val(TxtCommision.Text)
   Rs.Update
   Rs.Filter = ""
   If vIsNewRecord = True Then Call ActivityLog("Bank Machine", eAdd, TxtID.Text)
   FormStatus = NewMode
   Exit Sub
ErrorHandler:
   Call ShowErrorMessage
...
'      MsgBox "The Bank Machine ID must be two characters long", vbExclamation, "Alert"
'      If TxtID.Enabled And TxtID.Visible Then TxtID.SetFocus
'      Exit Function
'    End If
    If cn.Execute("Select count(*) from BankMachines where BankMachineID = " & TxtID.Text).Fields(0) > 0 Then
        MsgBox "This Bank Machine ID already exists. The Bank Machine ID must be unique", vbExclamation, "Alert"
        If TxtID.Enabled And TxtID.Visible Then TxtID.SetFocus
        Exit Function
    End If
...
'    End Select
  ...
  If Trim(TxtName.Text) = "" Then
    MsgBox "Please specify a Bank Machine name", vbExclamation, "Alert"
...
Private Sub Form_Load()
   ...
   SetWindowText Me.hWnd, "Bank Machines"
   HelpLocation Me
   Set Rs = New ADODB.Recordset
   Rs.Open "Select * FROM BankMachines", cn, adOpenDynamic, adLockOptimistic
...
Private Sub Grid_RowColChange(ByVal LastRow As Variant, ByVal LastCol As Integer)
   ...
   If Rs.RecordCount > 0 And Grid.Enabled Then
      ...
      TxtAccountNo.Text = Rs!AccountNo
...
Private Sub TxtFilter_Change()
   ...
   Rs.Find "BankMachineName like '" & Replace(TxtFilter.Text, "'", "''") & "%'", , adSearchForward, 1
...
Private Function FunGetMaxID() As String
  On Error GoTo ErrorHandler
  FunGetMaxID = cn.Execute("Select isnull(max(BankMachineID),0) + 1 from BankMachines").Fields(0)
  Exit Function
ErrorHandler:
  Call ShowErrorMessage
End Function
...
Private Sub UserActivities()
    ...
    With cn.Execute("Select  * from BankMachines where BankMachineID =" & TxtID.Text)
        If TxtName.Text <> !BankMachineName Then
            cn.Execute ("Insert Into UserActivities values ('BankMachines'" & "," & TxtID.Text & ", Null , 'Updated Bank Machine Name-" & !BankMachineName & "','" & Date & "','" & Time & "',2,'Updated'," & vUser & ")")
        End If
        If TxtAccountNo.Text <> !AccountNo Then
            cn.Execute ("Insert Into UserActivities values ('BankMachines'" & "," & TxtID.Text & ", Null , 'Updated Account No-" & !AccountNo & "','" & Date & "','" & Time & "',2,'Updated'," & vUser & ")")
        End If
        If TxtCommision.Text <> !Commision Then
            cn.Execute ("Insert Into UserActivities values ('BankMachines'" & "," & TxtID.Text & ", Null , 'Updated Commision-" & !Commision & "','" & Date & "','" & Time & "',2,'Updated'," & vUser & ")")
        End If
    End With
   Else
        cn.Execute ("Insert Into UserActivities values ('BankMachines'" & "," & TxtID.Text & ", Null ,'Saved','" & Date & "','" & Time & "',1,'Saved'," & vUser & ")")
   End If
End Sub
