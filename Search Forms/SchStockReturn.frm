...
Begin VB.Form SchStockReturn 
   ...
   Begin SSDataWidgets_B_OLEDB.SSOleDBGrid Grid 
      ...
      EndProperty
      stylesets(0).Picture=   "SchStockReturn.frx":0000
      AllowUpdate     =   0   'False
      MultiLine       =   0   'False
   ...
   End
End
Attribute VB_Name = "SchStockReturn"
...
Public ParaOutReturnDate As Date

Private Sub LoadGrid()
   On Error GoTo ErrorHandler
   Set Rs = New ADODB.Recordset
'   Rs.Open "SELECT StockReturnHeader.ReturnID, convert(varchar(10),ReturnDate,3) as ReturnDate, SalemanName as SalemanName" & vbCrLf _
'         + ",(TotalAmount+((TotalAmount*PerSalesTax)/100))-TotalDiscount as TotalAmount,TotalItems" & vbCrLf _
'         + " FROM StockReturnHeader INNER JOIN" & vbCrLf _
'         + " (SELECT ReturnID, count(ReturnID) as TotalItems FROM StockReturnBody GROUP BY ReturnID) StockReturnBody" & vbCrLf _
'         + " ON StockReturnHeader.ReturnID = StockReturnBody.ReturnID" & vbCrLf _
'         + " INNER JOIN Parties ON StockReturnHeader.PartyID = Parties.PartyID " & vbCrLf _
'         + " WHERE ReturnDate Between '" & DtpFrom.Value & "' AND '" & DtpTo.Value & "'" & vOrder & vDirection, CN, adOpenStatic, adLockReadOnly
   ...
   Rs.Open "SELECT StockReturnHeader.ReturnID, convert(varchar(10),ReturnDate,3) as ReturnDate, SalemanName as SalemanName" & vbCrLf _
         ...
         + " (SELECT ReturnID, count(ReturnID) as TotalItems FROM StockReturnBody GROUP BY ReturnID) StockReturnBody" & vbCrLf _
         + " ON StockReturnHeader.ReturnID = StockReturnBody.ReturnID" & vbCrLf _
         + " INNER JOIN Salesman ON StockReturnHeader.SalemanID = Salesman.SalemanID " & vbCrLf _
         + " WHERE ReturnDate Between '" & DtpFrom.Value & "' AND '" & DtpTo.Value & "'" & vOrder & vDirection, CN, adOpenStatic, adLockReadOnly
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
   ...
   If KeyCode = vbKeyReturn Then
      Select Case ActiveControl.Name
      Case Grid.Name, TxtReturnID.Name, DtpFrom.Name, DtpTo.Name
...
Select Case ColIndex
   Case 0
      vOrder = " order by StockReturnHeader.ReturnID"
   Case 1
      vOrder = " order by StockReturnHeader.ReturnDate"
   Case 2
