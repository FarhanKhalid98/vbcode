...
Private Sub BtnPrint_Click()
   ...
   vStrSQL = " Select D.*, StoreName from Demands D Left Outer Join Stores s on s.storeid = d.storeid" & vbCrLf _
            + " where D.DemandID = " & TxtDemandID.Text & " And D.DemandDate = '" & DtpDemandDate.DateValue & "'"

   If RsReport.State = adStateOpen Then RsReport.Close
   RsReport.Open vStrSQL, CN, adOpenStatic, adLockReadOnly
  
   Set RptReportViewer.Report = New CrptDemnadInvoice
...
Private Sub BtnDelete_Click()
  ...
  CN.BeginTrans
'  CN.Execute "Delete from DisputeInvoiceBody where DemandID = " & Val(TxtDemandID.Text)
  CN.Execute "Delete from Demands WHere DemandID = " & Val(TxtDemandID.Text)
  CN.CommitTrans
  FormStatus = NewMode
  Exit Sub
ErrorHandler:
...
Private Sub GetDemand()
   On Error GoTo ErrorHandler
   ssql = "Select D.*, StoreName from Demands D Left outer Join Stores s on s.storeid = D.storeid where DemandID = " & Val(TxtDemandID.Text) & " And DemandDate = '" & DtpDemandDate.DateValue & "'"
...
Private Sub BtnOpen_Click()
   SchDemands.Show vbModal
   If SchDemands.ParaOutDemandID <> Empty Then
      TxtDemandID.Text = SchDemands.ParaOutDemandID
      DtpDemandDate.DateValue = SchDemands.ParaOutDemandDate
      GetDemand
   End If
  Exit Sub
ErrorHandler:
...
Private Sub BtnSave_Click()
   ...
   If vIsNewRecord Then
      If CN.Execute("Select * from Demands where DemandID = " & Val(TxtDemandID.Text) & " And DemandDate = '" & DtpDemandDate.DateValue & "'").RecordCount > 0 Then
...
'   End If
  ...
  'Saving record
   ...
   CN.BeginTrans
   ssql = "Select * From Demands Where DemandID =" & Val(TxtDemandID.Text) & " And DemandDate ='" & DtpDemandDate.DateValue & "'"
   ...
   With Rs
      ...
      End If
         ...
         If Trim(TxtStoreID.Text) <> "" Then !StoreID = Val(TxtStoreID.Text)
...
Private Sub DtpDemandDate_Change()
    If DtpDemandDate.Visible = False Then Exit Sub
    If Me.ActiveControl.Name <> DtpDemandDate.Name Then Exit Sub
    TxtDemandID.Text = FunGetMaxID
    If DtpDemandDate.Enabled And DtpDemandDate.Visible Then FormStatus = ChangeMode
End Sub

Private Sub DtpDemandDate_Click()
    If DtpDemandDate.Visible = False Then Exit Sub
    If Me.ActiveControl.Name <> DtpDemandDate.Name Then Exit Sub
    TxtDemandID.Text = FunGetMaxID
End Sub
...
Private Sub Form_Load()
  ...
  'DtpDemandDate.Enabled = ObjUserSecurity.TaskAllowance("ChangeDateInCreditVoucher") Or ObjUserSecurity.IsAdministrator
...
Private Function FunGetMaxID() As Long
  On Error GoTo ErrorHandler
  FunGetMaxID = CN.Execute("Select isnull(max(DemandID),0) from Demands Where DemandDate = '" & DtpDemandDate.DateValue & "'").Fields(0) + 1
  Exit Function
ErrorHandler:
  Call ShowErrorMessage
End Function
